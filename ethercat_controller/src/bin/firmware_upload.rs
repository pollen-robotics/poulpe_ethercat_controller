...
fn main() -> Result<(), std::io::Error> {
    ...
    log::info!("Writing firmware to slave {:?} from file {}, with size {}", slave_pos, file_name, file_size);
    ...
    match mailbox_sdo_read(&mut master, idx, 0x100, 0x1, &mut data){
        Ok(_) => {
            ...
            if bytes_written == file_size as u32 {
                log::info!("Firmware written successfully, {} bytes written", bytes_written);
            } else {
                log::error!("Error: Firmware not written successfully, {} bytes written, expected {}", bytes_written, file_size);
                return Err(std::io::Error::new(std::io::ErrorKind::Other, "Firmware not written successfully"))
            }
        },
        Err(e) => 
        {
