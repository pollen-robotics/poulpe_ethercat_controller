...
fn main(){
    ...
    loop {
        ...
        let positions = ec.get_pdo_registers(id, &"position".into()).unwrap().iter().map(|x| f32::from_le_bytes(x[0..4].try_into().unwrap())).collect::<Vec<f32>>();
        let velocities = ec.get_pdo_registers(id, &"velocity".into()).unwrap().iter().map(|x| f32::from_le_bytes(x[0..4].try_into().unwrap())).collect::<Vec<f32>>();
        let torques = ec.get_pdo_registers(id, &"torque".into()).unwrap().iter().map(|x| f32::from_le_bytes(x[0..4].try_into().unwrap())).collect::<Vec<f32>>();
        let axis = ec.get_pdo_registers(id, &"axis_sensor".into()).unwrap().iter().map(|x| f32::from_le_bytes(x[0..4].try_into().unwrap())).collect::<Vec<f32>>();
        let target_position = ec.get_pdo_registers(id, &"target".into()).unwrap().iter().map(|x| f32::from_le_bytes(x[0..4].try_into().unwrap())).collect::<Vec<f32>>();
        log::info!("{:?}\t Tar: {:?},\t Pos: {:?}, \t Vel: {:?},\t  Tor: {:?},\t Axis: {:?}", t0.elapsed().as_millis(), target_position,positions, velocities, torques, axis);
        ...
        ec.set_pdo_register(id, &"velocity_limit".into(), 0, 1.0f32.to_le_bytes().to_vec());
